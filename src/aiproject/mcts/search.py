...
class Search:
    ...
    def play(self, simulation_time_in_s: float) -> np.ndarray:
        """
        ...
        this function will run for `simulation_time_in_s` : default 1sec
        """
        end_time: float = time.time() + simulation_time_in_s
        while True:
            to_simulate: Node = self._selection()
            simulation_result: int = to_simulate.simulate()
